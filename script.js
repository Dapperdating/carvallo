...
function carDetailUrl(car) {
  ...
  const basePath = /\/(auto|motors|selected)\.html$/i.test(currentPath) ? currentPath : "/auto.html";
...
function updateCatalogStats(cars) {
  const statCars = activeDivision === "all" ? cars : cars.filter((car) => car.division === activeDivision);
  const activeCount = statCars.filter((car) => !isArchivedStatus(car.status)).length;
  const archiveCount = statCars.filter((car) => isArchivedStatus(car.status)).length;
...
function scheduleHashAlignment() {
  if (!window.location.hash || window.location.hash.startsWith("#auto-")) return;
  alignHashTarget();
  [80, 300, 900, 1600].forEach((delay) => window.setTimeout(alignHashTarget, delay));
}
...
function initSite() {
  ...
  if (document.body.classList.contains("motors-page")) {
    activeDivision = "motors";
  }
